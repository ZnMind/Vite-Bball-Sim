...
const Matchup = () => {
    ...
    const playRound = () => {
        const tournament = roundRobin(teamlist);
        for (let i = 0; i < tournament.length; i++) {
            let round = tournament[i];
            for (let j = 0; j < round.length; j++) {
                let game = tournament[i][j];
                setTeam1(game[0]);
                setTeam2(game[1]);
                setMatch(match + 1);
    ...
    return (
        <>
            <div style={{ display: 'flex', justifyContent: 'space-around', width: '100%', borderLeft: '1px solid navy', borderRight: '1px solid navy' }}>
                <div>
                    <div className='box'>
                        <div>
                            <h3>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-start', width: '60%' }}>
                <button style={{ marginTop: '1em' }} onClick={() => getScore()}>Single</button>
                <button style={{ marginTop: '1em' }} onClick={() => playRound()}>Round Robin</button>
            </div>

            <div style={{ borderTop: '1px solid navy', width: '90%', marginTop: '2em' }}></div>
